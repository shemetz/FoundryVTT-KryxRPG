...
export const highlightCriticalSuccessFailure = function (message, html, data) {
  ...
  // Highlight rolls where the first part is a d20/2d10 roll
  const roll = message.roll;
  if (!roll.dice.length) return;
  const d = roll.dice[0];

  // Ensure it is an un-modified d20 roll
  const isBasicRoll = (basicDiceRoll === '1d20' && d.faces === 20) || (basicDiceRoll === '2d10' && d.faces === 20);
  if (!isBasicRoll) return;
  const isModifiedRoll = ("success" in d.results[0]) || d.options.marginSuccess || d.options.marginFailure;
  if (isModifiedRoll) return;

  // Highlight successes and failures
  let critical, fumble
  if (basicDiceRoll === '1d20') {
    critical = d.options.critical || 20;
    fumble = d.options.fumble || 1;
  } else {
    critical = d.options.critical || 18;
    fumble = d.options.fumble || 4;
  }
